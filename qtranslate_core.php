...
include_once( __DIR__ . '/src/Cookie.php' );
include_once( __DIR__ . '/src/Url.php' );

function qtranxf_init_language() {
	global $q_config, $pagenow;
	$q_config['cookie'] = $cookie = new Qtx\Cookie();
	...
	if ( empty( $_SERVER['REQUEST_URI'] ) ) {
		...
		if ( $_GET['lang'] ) {
			$url_info['language'] = qtranxf_sanitize_url( $_GET['lang'] );
		}
	}
...
function qtranxf_detect_language( &$url_info ) {
	...
	if ( ( !$lang || !isset( $url_info['doing_front_end'] ) )
		&& ( defined('DOING_AJAX') || !$q_config['cookie']->enabled )
...
function qtranxf_parse_language_info(&$url_info, $link=false) {
	...
	if ( !$link ) {
		...
		if( $lang ) {
			$url_info['lang_query_link'] = $lang;//todo excessive?
		}
	}
	if ( $lang ) {
		$cookie = new \Qtx\Cookie;
		$cookie->front = $lang;
...
function qtranxf_detect_language_front(&$url_info) {
	global $q_config;
	$lang = false;
	$determine_lang = function($url_info, $q_config) {
		$cookie = new \Qtx\Cookie();
		if( isset( $cookie->front ) ) {
			$lang = qtranxf_resolveLangCase( $cookie->front );
			if ( $lang ) {
				return $lang;
			}
		}
		$no_referrer = !isset( $_SERVER['HTTP_REFERER'] );
		$external_referrer = strpos( $_SERVER['HTTP_REFERER'], $url_info['host'] ) === false;
		$is_home = empty( $url_info['wp-path'] ) || $url_info['wp-path'] == '/';
		if ( $q_config['detect_browser_language'] && ($no_referrer || $external_referrer ) && $is_home ) {
	...
	$lang = $determine_lang($url_info, $q_config);
	if ( !isset($url_info['doredirect'])
...
function qtranxf_resolveLangCase($lang,&$caseredirect = null){
...
function qtranxf_url_set_language( $urlinfo, $lang, $showLanguage ) {
	...
	$url = new \Qtx\Url( $urlinfo );
	if ( $showLanguage ) {
		$url_mode = $q_config['url_mode'];
		switch( $url_mode ) {
			case QTX_URL_PATH:
				$url->setPath( $lang );
				$urlinfo['wp-path'] = $url->path;
				break;
			case QTX_URL_DOMAIN:
				$url->setHost( $lang );
				...
				break;
			case QTX_URL_DOMAINS:
				// @todo Figure out best way to add this functionality to the Qtx\Url class
				$urlinfo['host'] = $q_config['domains'][$lang];
				break;
			case QTX_URL_QUERY:
				$url->setQuery( $lang );
				$urlinfo['query'] = $url->query;
				$urlinfo['wp-path'] = $url->path;
				break;
			default:
				break;
